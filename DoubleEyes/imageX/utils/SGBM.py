...
def SGBM_update(img_l_in, img_r_in, img_out):
    """
    ...
    imgL = cv2.imread(img_l_in)
    imgR = cv2.imread(img_r_in)
    # imgL = cv2.resize(imgL, None, fx=0.125, fy=0.125, interpolation=cv2.INTER_CUBIC)
    # imgR = cv2.resize(imgR, None, fx=0.125, fy=0.125, interpolation=cv2.INTER_CUBIC)
    ...
    # cv2.imshow('SGNM_disparity1', (disp-min_disp)/num_disp)

    cv2.imwrite(img_out, dst)


if __name__ == "__main__":
    SGBM_update(img_l_in=r'E:\left_rectify.jpg', img_r_in=r'E:\right_rectify.jpg', img_out=r'E:\out.png')
    pass
